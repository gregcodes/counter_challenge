...
# store single bids into new data table
exclude <- duplicates$X
single_bids <- dataset[-exclude]
single_bids$Count <- 1

# merge single bids with multiple bids that were counted
total <- rbind(single_bids, duplicates)
total <- total[Count==1]
total <- total[order(Target.Name, Date.Announced, decreasing=FALSE)]
des_columns <- c(2:3)
total <- total[, des_columns, with=FALSE]
